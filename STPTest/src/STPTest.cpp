...
int main() {
	ns_utils::timer_ptr_t ptr = std::make_shared < CMyTimerHandler
			> (ns_utils::ETimerType::ETT_REALTIME, 1, 1);
	...
	ptr = std::make_shared < CMyTimerHandler
			> (ns_utils::ETimerType::ETT_MONOTONIC, 2, 1);
	tms.register_timer(ptr);

	ns_utils::timer_ptr_t ptr1 = std::make_shared < CMyTimerHandler
			> (ns_utils::ETimerType::ETT_REALTIME, 3, 1);
	tms.register_timer(ptr1);

	ns_utils::timer_ptr_t ptr2 = std::make_shared < CMyTimerHandler
			> (ns_utils::ETimerType::ETT_MONOTONIC, 5, 1);
